...
class KITTIStereo2015(Dataset):
    def __init__(self, args, mode, split='train'):
        super(KITTIStereo2015, self).__init__()
        self.args = args["data"]
        self.mode = mode
    ...
    def _load_data(self, idx):
        ...
        P_rect_02 = np.reshape(calib['P_rect_02'], (3, 4))      # 相机 02 的投影矩阵
        P_rect_03 = np.reshape(calib['P_rect_03'], (3, 4))      # 相机 03 的投影矩阵
    ...
    def __getitem__(self, idx):
        ...
        return {"ir1":img1, "ir2":img2, "gt1":gt1,  "gt2":[], "mask1": mask1, "mask2": [],
                ...
                "ref_path": [] if self.mode == 'train' else one_sample['ref_path'], # 仅在测试真实数据的时候需要用到
                }
