...
class DrivingStereo(Dataset):
    def __init__(self, args, mode, split='train'):
        ...
        if self.args['augment']:
            self.augmentation = augmentation.Compose([
                                ...
                                augmentation.RandomCrop(self.args['crop_size'], shift=True,
    ...
    def _load_data(self, idx):
        ...
        # 载入标定参数(TODO:有待检查, 不影响视差结果)
        calib = self.load_calib(path_calib)
        P_rect_02 = np.reshape(calib['P_rect_101'], (3, 4))      # 相机 02 的投影矩阵
        P_rect_03 = np.reshape(calib['P_rect_103'], (3, 4))      # 相机 03 的投影矩阵
