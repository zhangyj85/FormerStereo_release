...
def forward_swin(pretrained, x):
    ...
    # 将 token 转为图像, 得到的 layer 特征大小为 [1/4, 1/8, 1/16, 1/32], c=Dim
    unflatten1 = nn.Unflatten(2, torch.Size([
        h // pretrained.model.patch_size[1] // 2 ** 0,
        w // pretrained.model.patch_size[0] // 2 ** 0,
    ]))
    ...
    unflatten4 = nn.Unflatten(2, torch.Size([
        h // pretrained.model.patch_size[1] // 2 ** 3,
        w // pretrained.model.patch_size[0] // 2 ** 3,
    ]))

    if layer_1.ndim == 3:
        layer_1 = unflatten1(layer_1)
    if layer_2.ndim == 3:
        layer_2 = unflatten2(layer_2)
    if layer_3.ndim == 3:
        layer_3 = unflatten3(layer_3)
    if layer_4.ndim == 3:
        layer_4 = unflatten4(layer_4)
...
def forward_flex(self, x):
    ...
    # 将 [cls] token 加入到 token 序列中: Swin Transformer 不存在 [CLS] Token
    ...
    elif getattr(self, "cls_token", None) is not None:
        cls_tokens = self.cls_token.expand(
            B, -1, -1
        )  # stole cls_tokens impl from Phil Wang, thanks
    ...
    x = x + pos_embed
    if hasattr(self, "pos_drop"):   # swin 做完位置编码后有 dropout
        x = self.pos_drop(x)

    i = 0
    for blk in self.layers:
        x = blk(x, h // patch_H // 2 ** i, w // patch_W // 2 ** i)
        i += 1
...
def _make_swin_b16_backbone(
    ...
    pretrained.model.layers[0].blocks[-1].register_forward_hook(get_activation("1"))
    pretrained.model.layers[1].blocks[-1].register_forward_hook(get_activation("2"))
    pretrained.model.layers[2].blocks[-1].register_forward_hook(get_activation("3"))
    pretrained.model.layers[3].blocks[-1].register_forward_hook(get_activation("4"))

    pretrained.activations = activations

    if enable_attention_hooks:
        # 这个 hock 应该是用来可视化看网络关注了那些地方?
        pretrained.model.layers[0].blocks[-1].attn.register_forward_hook(
            get_attention("attn_1")
        )
        pretrained.model.layers[1].blocks[-1].attn.register_forward_hook(
            get_attention("attn_2")
        )
        pretrained.model.layers[2].blocks[-1].attn.register_forward_hook(
            get_attention("attn_3")
        )
        pretrained.model.layers[3].blocks[-1].attn.register_forward_hook(
...
def _make_pretrained_swinmim_swin4_224(
    use_readout="ignore",
    hooks=None,
    enable_attention_hooks=False
):
    ...
    # 加载权重
    path = "fundation_model_weights/simmim_finetune__swin_large__img224_window14__800ep.pth"
    if os.path.exists(path):
        states = torch.load(path, map_location="cpu")
        weights = states['model']
        model.load_state_dict(weights, strict=True)
    else:
        warning(f"No valid Swim pretrained model in path: {path}")
